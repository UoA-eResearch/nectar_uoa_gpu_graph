    ...
    RGraph.Drawing.YAxis = function (conf)
    {
        ...
        this.getShape = function (e)
        {
            ...
            if (   mouseX >= this.x - (prop.tickmarksAlign ==  'right' ? 0 : this.getWidth())
                ...
                if( Array.isArray(prop.yaxisLabelsSpecific) ){
                  units = (h / prop.yaxisLabelsSpecific.length);
                  tip_index = Math.floor((mouseY - y)/units);
                }
                
                if (RGraph.parseTooltipText && prop.tooltips) {
                    var tooltip = RGraph.parseTooltipText(prop.tooltips, tip_index);
                }
